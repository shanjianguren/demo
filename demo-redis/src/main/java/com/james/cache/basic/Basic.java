...
public class Basic {
	public static String ip = "116.62.163.237";
	public static int port = 6379;
	public static String auth = "1234qwer";
	...
	*/
	...
	public static String hong="" +
			"if redis.call('sismember', KEYS[3],ARGV[1]) ~= 0 then\n" +
					"return nil\n" +
			"else\n" +
				"local hb=redis.call('rpop', KEYS[1]);\n" +
				"if hb then\n" +
					"local xx=cjson.decode(hb);\n " +
                    "xx['userId']=ARGV[1];\n" +
			        "local result=cjson.encode(xx);\n" +
                    "redis.call('sadd',KEYS[3],ARGV[1]);\n" +
                    "redis.call('lpush',KEYS[2],result);\n" +
                    "return result;\n"+
                "end\n"+
            "end\n" +
            "return nil"
